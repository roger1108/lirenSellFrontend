<template>
	<div class="payment">
	  <div class="user-info">
			...
			<!--roger change <div class="item"><label>送餐地址</label><input placeholder="送餐地址" v-model="address" type="text"></div> -->
		</div>
		<div class="food-info">
		...
		<div class="footer">
			...
			<!-- roger change begin -->
			<div class="btn-pay" @click="pay">确认 </div>
			<!-- roger change end -->
...
<script >
	...
	export default {
		...
		created() {
			...
			this.phone = getCookie('buyer_phone') || "";
			this.name = getCookie('buyer_name') || "";
		},
		methods: {
			pay() {
                ...
                this.$http.post("/sell/buyer/order/create", {
                    ...
                    'phone': this.phone,
                    'name': this.name,
                    //'address': this.address,  //roger change
			...
			// roger change begin -->
						...
						document.cookie = 'buyer_phone=' + this.phone + ";expires=" + exp.toGMTString();
						document.cookie = 'buyer_name=' + this.name + ";expires=" + exp.toGMTString();
